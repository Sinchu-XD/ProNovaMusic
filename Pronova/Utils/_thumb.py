...
class Thumbnail:
    ...
    async def save_thumb(self, output_path: str, url: str) -> str:
        ...
        try:
            ...
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.get(DEFAULT_THUMB, timeout=aiohttp.ClientTimeout(total=15)) as resp:
                        data = await resp.read()
                        if data:
                            with open(output_path, "wb") as f:
                                f.write(data)
            except Exception as fallback_err:
                LOGGER.error(f"Default thumb download also failed: {fallback_err}")

        return output_path

    async def generate(self, song: Track, size=(1280, 720)) -> str:
        try:
            song_id = hashlib.md5((song.url or song.title or "unknown").encode()).hexdigest()
            ...
            if not os.path.exists(temp) or os.path.getsize(temp) < 5000:
                LOGGER.warning(f"Downloaded thumbnail invalid for {song.title}, returning default URL")
    ...
    def _generate_sync(self, temp: str, output: str, song: Track, size=(1280, 720)) -> str:
        try:
            ...
            clean_title = re.sub(r"\W+", " ", song.title or "Unknown").title()
            draw.text(
                (TITLE_X, TITLE_Y),
            ...
            draw.text(
                (TITLE_X, META_Y),
                f"{song.channel or 'Unknown'} | {song.views or 'Unknown Views'}",
                fill="black",
            ...
            draw.text(
                (BAR_X, BAR_Y + 15), "00:00", fill="black", font=self.regular_font
            )

            is_live = getattr(song, "is_live", False)
            end_text = "Live" if is_live else (song.duration_text or "0:00")
            draw.text(
            ...
            if os.path.isfile(icons_path):
                try:
                    ...
                    LOGGER.warning(f"Icons load failed: {icon_err}")

            bg.save(output)

            if not os.path.exists(output) or os.path.getsize(output) < 10000:
                raise Exception("Generated thumbnail invalid or too small")

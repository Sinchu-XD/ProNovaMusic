...
def build_caption(title, url, duration, requester, header="Nᴏᴡ Pʟᴀʏɪɴɢ", position=None):
    try:
        ...
        if hasattr(requester, "id"):
            ...
            name = clean_html(requester.first_name or "User")
        else:
            user_id = None
...
class Plugin:
    ...
    async def on_song_start(self, chat_id, song):
        ...
        initial_bar = (
            f"00:00 🔘▬▬▬▬▬▬▬▬▬ {format_time(duration_sec)}"
            if duration_sec > 0
            else "00:00 ▬▬▬▬▬▬▬▬▬▬ 00:00"
        )

        try:
            thumb_path = await generate(song)
            ...
            if duration_sec > 0:
                task = asyncio.create_task(
                    self.progress_loop(chat_id, msg, duration_sec)
                )
    ...
    async def on_seek(self, chat_id, song, seconds):
        ...
        try:
            ...
            if duration_sec > 0:
                task = asyncio.create_task(
                    self.progress_loop(chat_id, new_msg, duration_sec)
                )
                self.progress_tasks[chat_id] = task

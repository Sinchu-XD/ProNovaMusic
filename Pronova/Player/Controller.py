...
class VoiceController:
    ...
    async def play(self, chat_id, query, requested_by, video: bool = False):
        ...
        for data in results:
            ...
            try:
                ...
                LOGGER.info(f"[FINAL STREAM] {song.title} -> {song.stream[:60]}...")

                pos = await self.player.play(chat_id, song, video=song.is_video)

            except Exception as e:
                # Re-raise so callers (play.py) can inspect the original error
    ...
    async def _on_end(self, chat_id):
        ...
        try:
            ...
            try:
                ...
                LOGGER.error(f"[SKIP IN ON_END ERROR]\n{format_exc()}")
                return
    ...
    async def _on_vc_closed(self, chat_id):
        ...
        try:
            ...
            LOGGER.error(f"[VC_CLOSED STOP ERROR]\n{format_exc()}")

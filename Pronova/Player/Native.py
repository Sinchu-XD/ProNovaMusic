...
class _NativeEngine:
    def __init__(self, app):
        ...
        async def _ended(_, update: StreamEnded):
            LOGGER.info(f"[STREAM END EVENT] Chat: {update.chat_id}")

            try:
                if self.on_end:
                    await self.on_end(update.chat_id)
                else:
                    LOGGER.warning(f"[NO on_end HANDLER] Chat: {update.chat_id}")
            except Exception:
        ...
        async def _vc_closed(_, update: ChatUpdate):
            LOGGER.warning(f"[VC CLOSED EVENT] Chat: {update.chat_id}")

            try:
                if self.on_vc_closed:
                    await self.on_vc_closed(update.chat_id)
                else:
                    LOGGER.warning(f"[NO VC CLOSED HANDLER] Chat: {update.chat_id}")
            except Exception:
    ...
    async def play(
        ...
        LOGGER.info(f"[PLAY REQUEST] Chat: {chat_id} | Stream: {stream} | Video: {video}")

        if not stream:
            LOGGER.error(f"[EMPTY STREAM] Chat: {chat_id}")
        ...
        if isinstance(start_time, int) and start_time > 0:
            ...
            LOGGER.debug(f"[SEEK PARAM] {ffmpeg_params}")
        ...
        try:
            ...
            LOGGER.debug(f"[MEDIA CREATED] Chat: {chat_id}")
            ...
            LOGGER.error(f"[PLAY ERROR] Chat: {chat_id}\n{format_exc()}")
            raise
    ...
    async def stop(self, chat_id):
        ...
        try:
            ...
            LOGGER.error(f"[STOP ERROR] Chat: {chat_id}\n{format_exc()}")
    ...
    async def change_volume(self, chat_id, volume: int = 200):
        ...
        LOGGER.info(f"[VOLUME CHANGE] Chat: {chat_id} | Volume: {volume}")

...
async def get_stream_cache(key, logger=None):
    try:
        data = await Core.db.yt_stream_cache.find_one({"_id": key})
        if not data:
            return None
        ...
        if expire_time:
            ...
            if remaining <= 120:
                await Core.db.yt_stream_cache.delete_one({"_id": key})
                return None
...
async def set_stream_cache(key, stream):
    try:
        ...
        await Core.db.yt_stream_cache.update_one(
            {"_id": key},
            {
                "$set": {
                    "stream": stream,
...
async def get_search_cache(key):
    try:
        data = await Core.db.yt_search_cache.find_one({"_id": key})
        if not data:
            return None
...
async def set_search_cache(key, data):
    try:
        await Core.db.yt_search_cache.update_one(
            {"_id": key},
            {
                "$set": {
                    "data": data,

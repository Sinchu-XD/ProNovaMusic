...
from Pronova.Database import (
    total_users, total_chats, get_lifetime, inc_lifetime,
    total_banned, sum_range, top_groups, top_song_players, most_played,
)
from Pronova.Database import Core as _db_core
...
async def stats(client, m):
    try:
        ...
        try:
            ...
            gbanned = await _db_core.db.gbanned.count_documents({})
            ...
            total_sudo = await _db_core.db.sudo_users.count_documents({})
            total_auth = await _db_core.db.auth_users.count_documents({})
